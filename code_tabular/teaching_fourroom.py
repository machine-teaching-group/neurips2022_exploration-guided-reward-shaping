...
class teaching():
    ...
    def end_to_end_training(self):
        ...
        # traj = utils.sample_trajectory_given_state_action(env_orig, self.agent, start_state=2,
        #                                                   start_action=1, epsilon_reinforce=0.0)
        #
        # print(traj)
        # exit(0)
        ...
        for i in range(N):
            ...
            if i % agent_evaluation_step == 0:
                ...
                print("===================================================")
                # print(self.agent.actor_policy)

            # if i > 500 or (np.round(expected_reward_array_env_orig[-1], 4) <= 0.3 and i > 500):
            #     # print(self.env_teacher.phi_sors)
            #     print(self.env_teacher.phi_sors)
            #     print(self.agent.actor_policy)
            #     print("===============Iter = {}/{}========================".format(i, N))
            #     print("Teacher = {}".format(self.teacher_name))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--n2_subgoal', default=0.0, type=float, help='n2 subgoal')
    parser.add_argument('--n_averaged', default=10, type=int, help='N runs to average')

    args = parser.parse_args()
    agent_type = args.agent

    n2_subgoal = args.n2_subgoal

    output_directory = "runs_room/agent={}_n2_subgoal={}".format(args.agent, args.n2_subgoal)
    ...
    for i in range(0, args.n_averaged):
        ...
        for gridsizefull in [7]:

            env_args = {
                "gridsizefull": gridsizefull,
                "R_max": 1,
                "gamma": 0.99,
                "randomMoveProb": 0.05,
                "n2_subgoal": n2_subgoal,
                "H": 30
            }

            env_orig = env_fourroom.Environment(env_args)
            dict_accumulator = {}
    ...
    fig_name = "room_wo_distractor_reinforce" if args.n2_subgoal == 0.0 else "room_with_distractor_reinforce"
    plot_results.plot_reinforce_curves(path, out_folder_name, fig_name, n_file=args.n_averaged, t=5100, four_room_flag=True)
